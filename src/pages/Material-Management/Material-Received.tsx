...
const MaterialReceived = () => {
  ...
  const sections: Section[] = [
    { title: 'Material Received Listing', key: 'listing' },
    { title: 'Material Received Creation', key: 'creation' },
    ...(isEditing ? [{ title: `Edit Material Received - ${transaction?.transactionNumber}`, key: 'edit' }] : []),
  ]
    ...
    <PageWapper className="!bg-transparent !shadow-none">
      ...
      {activeTab === 'listing' && (
        <div className="flex flex-col gap-4 bg-white p-4 rounded-lg h-fit">
          <h1 className="text-2xl font-medium text-zinc-1300 uppercase">Material Received</h1>
      ...
      {activeTab === 'creation' && (
        <CreateBox
          title={"Material Received Creation"}
          setActiveTab={setActiveTab}
          items={items}
          setItems={setItems}
          type={13}
          fetchData={() => fetchTransactionsFromDB(13, setData)}
        />
      )}
