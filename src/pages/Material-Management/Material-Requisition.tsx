...
const MaterialRequisition = () => {
  ...
  const sections: Section[] = [
    { title: 'Material Requisition Listing', key: 'listing' },
    { title: 'Material Requisition Creation', key: 'creation' },
    ...(isEditing ? [{ title: `Edit Material Requisition - ${transaction?.transactionNumber}`, key: 'edit' }] : []),
  ]

  useEffect(() => {
    handleManipulateDropdown(11, true, false)
    fetchTransactionsFromDB(11, setData)
  }, [])
    ...
    <PageWapper className="!bg-transparent !shadow-none">
      ...
      {activeTab === 'listing' && (
        <div className="flex flex-col gap-4 bg-white p-4 rounded-lg h-fit">
          <h1 className="text-2xl font-medium text-zinc-1100 uppercase">Material Requisition</h1>
      ...
      {activeTab === 'creation' && (
        <CreateBox
          title={"Material Requisition Creation"}
          setActiveTab={setActiveTab}
          items={items}
          setItems={setItems}
          type={11}
          fetchData={() => fetchTransactionsFromDB(11, setData)}
        />
      )}

      {activeTab === 'edit' && (
        <EditBox
          title={`Edit Material Requisited - ${transaction?.transactionNumber}`}
          setActiveTab={setActiveTab}
