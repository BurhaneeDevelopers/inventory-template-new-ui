...
const ProductionSlip = () => {
  ...
  const sections: Section[] = [
    { title: 'Production Slip Listing', key: 'listing' },
    { title: 'Production Slip Creation', key: 'creation' },
    ...(isEditing ? [{ title: `Edit Production Slip - ${transaction?.transactionNumber}`, key: 'edit' }] : []),
  ]

  useEffect(() => {
    handleManipulateDropdown(15, true, false)
    fetchTransactionsFromDB(15, setData)
  }, [])
    ...
    <PageWapper className="!bg-transparent !shadow-none">
      ...
      {activeTab === 'listing' && (
        <div className="flex flex-col gap-4 bg-white p-4 rounded-lg h-fit">
          <h1 className="text-2xl font-medium text-zinc-1500 uppercase">Production Slip</h1>
      ...
      {activeTab === 'creation' && (
        <CreateBox
          title={"Production Slip Creation"}
          setActiveTab={setActiveTab}
          items={items}
          setItems={setItems}
          type={15}
          fetchData={() => fetchTransactionsFromDB(15, setData)}
        />
      )}

      {activeTab === 'edit' && (
        <EditBox
          title={`Edit Production Slip - ${transaction?.transactionNumber}`}
          setActiveTab={setActiveTab}
