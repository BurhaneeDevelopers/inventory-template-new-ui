...
const InternalOrder = () => {
    ...
    const sections: Section[] = [
        { title: 'Internal Order Listing', key: 'listing' },
        { title: 'Internal Order Creation', key: 'creation' },
        ...(isEditing ? [{ title: `Edit Internal Order - ${transaction?.transactionNumber}`, key: 'edit' }] : []),
    ]

    useEffect(() => {
        handleManipulateDropdown(10, true, false)
        fetchTransactionsFromDB(10, setData)
    }, [])
    ...
    return (
        <PageWapper className="!bg-transparent !shadow-none">
            ...
            {activeTab === 'creation' && (
                <CreateBox
                    title={"Internal Order Creation"}
                    setActiveTab={setActiveTab}
                    items={items}
                    setItems={setItems}
                    type={10}
                    fetchData={() => fetchTransactionsFromDB(10, setData)}
                />
            )}

            {activeTab === 'edit' && (
                <EditBox
                    title={`Edit Internal Order - ${transaction?.transactionNumber}`}
                    setActiveTab={setActiveTab}
