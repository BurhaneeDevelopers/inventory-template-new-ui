    ...
    return (
        ...
        {entries?.map((entry, index) => {
            ...
            <tr key={`entry_${index}`}>
                ...
                <td className="Col-description">{entry.item.itemName} - {entry.itemDescription}</td>
                ...
                <td className="Col-total">{<>&#8377;{(entry.totalPrice.toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</>}</td>
            </tr>
            )
        })}
        <tr>
            ...
            <td className="Col-total">{<>&#8377;{(entries?.reduce((acc: number, entry: any) => acc + entry.totalPrice, 0).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}}</>}</td>
        </tr>
        <tr>
            <td colSpan={4} style={{borderBottom:"0px"}}></td>
            <td className="Col-total">{'TAX'}</td>
            <td className="Col-total">{<>&#8377;{(entries?.reduce((acc: number, entry: any) => acc + entry.taxAmount, 0).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}}</>}</td>
        </tr>
        <tr>
            <td colSpan={4} style={{borderBottom:"0px"}}></td>
        ...
        <tr>
            ...
            <td className="Col-total">{<>&#8377;{(entries?.reduce((acc: number, entry: any) => acc + entry.totalPrice + entry.taxAmount, 0).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</>}</td>
        </tr>
        </tbody>
        </table>
    )
