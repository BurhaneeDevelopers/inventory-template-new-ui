...
const EditBoxTable = ({ transaction, setTransaction, handleDeleteItem, totalTax, totalPrice, grandTotal }) => {
    ...
    const handleSave = () => {
        ...
        const updatedItems = [...transaction.detail];
        const currentItem = updatedItems[editIndex];

        const quantity = Number(editedValues.quantity || currentItem.quantity);
        const unitPrice = Number(editedValues.unitPrice || currentItem.unitPrice);
    ...
    return (
        <div>
                                        ...
                                        if (isEditing && (field.id === 'quantity' || field.id === 'unitPrice')) {
                                            return (
                                                <TableCell key={field.id}>
                                                    <Input
                                                        value={editedValues[field.id] || value || ''}
                                                        onChange={(e) =>
            ...
            <div className="flex flex-row ml-auto p-4 gap-7 bg-white mt-4 w-fit rounded-lg">
                <div className="flex items-end transaction-end gap-2">
                    <h4 className="text-base">Total Tax Amount</h4>
                    <h3 className="text-2xl font-medium">{totalTax.toFixed(2)}</h3>
                </div>
                <div className="flex items-end transaction-end gap-2">
